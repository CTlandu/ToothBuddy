...
- [ ] **D2 接电话/Siri/闹钟也暂停(不改 scenePhase 的中断)。** session 进行中,让另一台设备打来电话(或触发 Siri / 响个闹钟)打断音频但**不离开 app**。期望:计时**暂停**、语音停;中断结束后计时继续。**不应**出现"语音被系统静音了但计时还在涨"。
  > ⚠️ **已知 gap(code-review 标记,待你验证后决定是否补)**:app 现在没有主动 `setActive` 自己的 `AVAudioSession`,`AVSpeechSynthesizer` 只在**正在念**的时候才持有音频会话。所以**中断如果发生在两条语音提示之间的静默段**,系统可能不发 `interruptionNotification`,这次中断就**不会**触发暂停(计时会膨胀)。**答应的来电会切后台(D1 路径仍能暂停)**,所以主场景 OK;漏的是"前台静默段被 Siri/闹钟/配对设备来电打断"。要彻底修需要给 session 起一个 `.playback` + `.duckOthers` 的音频会话(会改动和你背景音乐的交互——念提示时压低音乐),这有副作用、需要真机调,所以没盲改。真机验这条时如果发现静默段中断没暂停,就是这个 gap,回报我再补。
